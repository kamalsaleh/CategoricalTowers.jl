...
InstallMethodWithCache( AbelianClosure,
  ...
  function( algebroid, range_category_of_hom_structure )
    ...
    if (!( HasIsAbelianCategory( range_category_of_hom_structure ) && IsAbelianCategory( range_category_of_hom_structure ) ))
        TryNextMethod( );
    end;
...
@InstallMethod( AbelianClosure,
  ...
  function( algebroid )
    ...
    if (@not HasRangeCategoryOfHomomorphismStructure( algebroid ))
        TryNextMethod( );
    end;
...
@InstallMethod( /,
  ...
  function( name, abelian_closure )
    ...
    if (IsObjectInAbelianClosure( Yc ))
        
        SetIsProjective( Yc, true );
        
    elseif (IsMorphismInAbelianClosure( Yc ))
        
        if (CanCompute( abelian_closure, "IsMonomorphism" ))
            IsMonomorphism( Yc );
        end;
        
        if (CanCompute( abelian_closure, "IsSplitMonomorphism" ))
            IsSplitMonomorphism( Yc );
        end;
        
        if (CanCompute( abelian_closure, "IsEpimorphism" ))
            IsEpimorphism( Yc );
        end;
        
        if (CanCompute( abelian_closure, "IsSplitEpimorphism" ))
            IsSplitEpimorphism( Yc );
        end;
        
        ## IsIsomorphism == IsSplitMonomorphism and IsSplitEpimorphism
        ## we add this here in case the logic is deactivated
        if (CanCompute( abelian_closure, "IsIsomorphism" ))
            IsIsomorphism( Yc );
        end;
