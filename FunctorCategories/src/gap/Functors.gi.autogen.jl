...
@InstallMethod( NakayamaLeftAdjointData,
  ...
  function ( coPSh )
    ...
    #% CAP_JIT_DROP_NEXT_STATEMENT
    if (@not CanCompute( H, "DualOnMorphisms" ))
        TryNextMethod( );
    end;
    ...
    Nakayama_functor_on_objs =
      function ( F )
        ...
        return CreateCoPreSheafByValues( coPSh,
                    PairGAP( ValuesOfPreSheaf( F )[1], List( ValuesOfPreSheaf( F )[2], m -> DualOnMorphisms( H, m ) ) ) );
    ...
    return PairGAP( Nakayama_functor_on_objs, Nakayama_functor_on_mors );
...
@InstallMethod( NakayamaRightAdjointData,
...
@InstallMethod( IsbellLeftAdjointData,
  ...
  function ( PSh, coPSh )
    ...
    return PairGAP( IsbellLeftAdjoint_on_objs, IsbellLeftAdjoint_on_mors );
...
@InstallMethod( IsbellRightAdjointData,
  ...
  function ( coPSh, PSh )
    ...
    return PairGAP( IsbellRightAdjoint_on_objs, IsbellRightAdjoint_on_mors );
...
@InstallMethod( UnitOfIsbellAdjunctionData,
...
@BindGlobal( "SET_ISOMORPHISMS_BETWEEN_SOURCE_AND_IMAGE_OF_YONEDA_EMBEDDING_OF_SOURCE",
  ...
  function ( PSh )
    ...
    if (@not IsCategoryOfRows( Target( PSh ) ))
