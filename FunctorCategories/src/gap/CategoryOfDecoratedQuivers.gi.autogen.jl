...
InstallMethodWithCache( CategoryOfDecoratedQuivers,
  ...
  function ( decorating_quiver, decoration_of_vertices, decoration_of_arrows )
    ...
    if (@not HasDefiningTripleOfQuiverEnrichedOverSkeletalFinSets( decorating_quiver ))
    ...
    modeling_tower_object_datum =
      function( DecoratedQuivers, obj )
        ...
        return PairGAP( ObjectDatum( Quivers, Source( morphism ) ),
...
@InstallMethod( CreateDecoratedQuiver,
  ...
  function ( category_of_quivers, decoration_of_vertices, arrows_with_decoration )
    ...
    if (ForAll( arrows_with_decoration, IsList ))
        arr = [ List( arrows_with_decoration, e -> e[[ 1, 2 ]] ), List( arrows_with_decoration, e -> e[3] ) ];
    else
        @Assert( 0, 0 == Length( arrows_with_decoration ) mod 3 );
        arr = [ List( (1):(Length( arrows_with_decoration ) / 3), i -> PairGAP( arrows_with_decoration[3 * i - 2], arrows_with_decoration[3 * i - 1] ) ),
                 List( (1):(Length( arrows_with_decoration ) / 3), i -> arrows_with_decoration[3 * i] ) ];
    end;
    
    return CreateDecoratedQuiver( category_of_quivers,
                   PairGAP( Triple( Length( decoration_of_vertices ), Length( arr[1] ), arr[1] ),
                         PairGAP( decoration_of_vertices, arr[2] ) ) );
...
@InstallMethod( CreateDecoratedQuiverMorphism,
  ...
  function ( source, images_of_vertices, images_of_arrows, range )
    ...
    return CreateDecoratedQuiverMorphism( CapCategory( source ), source, PairGAP( images_of_vertices, images_of_arrows ), range );
...
@InstallMethod( Subobject,
  ...
  function ( decorated_quiver, images_of_vertices, images_of_arrows )
    ...
    vertices = AsList( decorated_quiver.V )[1 + SetGAP( @Concatenation( images_of_vertices, @Concatenation( arrows_as_pairs ) ) )];
...
@InstallMethod( EmbeddingOfUnderlyingCategory,
  ...
  function ( category_of_quivers )
    ...
    AddObjectFunction( U,
            F -> CreateDecoratedQuiver( category_of_quivers, Triple( Cardinality( F.V ), Cardinality( F.A ), ListN( AsList( F.s ), AsList( F.t ), ( s, t ) -> PairGAP( s, t ) ) ) ) );
    
    AddMorphismFunction( U,
            ( source, eta, range ) -> CreateDecoratedQuiverMorphism( category_of_quivers, source, PairGAP( AsList( eta.V ), AsList( eta.A ) ), range ) );
...
@InstallMethod( \.,
  ...
  function ( category_of_quivers, string_as_int )
    ...
    if (IsObjectInCategoryOfDecoratedQuivers( Yc ))
        
        SetIsProjective( Yc, true );
        
    elseif (IsMorphismInCategoryOfDecoratedQuivers( Yc ))
...
MakeShowable( [ "image/svg+xml" ], IsMorphismInCategoryOfDecoratedQuivers && IsMonomorphism );
...
@InstallMethod( DotVertexLabelledDigraph,
  ...
  function ( decorated_quiver )
    ...
    for i in AsList( decorated_quiver.V )
    ...
    for i in AsList( decorated_quiver.A )
...
@InstallMethod( DotVertexLabelledDigraph,
        ...
        [ IsMorphismInCategoryOfDecoratedQuivers && IsMonomorphism ],
