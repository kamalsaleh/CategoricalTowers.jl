...
##
@InstallMethod( CreatePreSheafByValues,
...
@InstallMethod( CreatePreSheafByValues,
  ...
  function ( PSh, values_of_all_objects, values_of_all_generating_morphisms )
    ...
    return CreatePreSheafByValues( PSh, PairGAP( values_of_all_objects, values_of_all_generating_morphisms ) );
    
end );

##
@InstallMethod( CreatePreSheafByFunctions,
...
@InstallMethod( CreatePreSheaf,
  ...
  function ( B, images_of_objects, images_of_morphisms )
    ...
    if (IsPreSheafCategory( B ))
        TryNextMethod( );
    elseif (IsFunctorCategory( B ))
        TryNextMethod( );
    elseif (IsEmpty( images_of_objects ))
...
@InstallMethod( CreatePreSheaf,
        "for a presheaf category and two lists",
        [ IsPreSheafCategory && HasRangeCategoryOfHomomorphismStructure, IsList, IsList ],
...
##
@InstallMethod( CreatePreSheafMorphismByValues,
...
##
@InstallMethod( CreatePreSheafMorphismByFunction,
  ...
  function ( PSh )
    ...
    AddIsEqualForObjects( PSh,
      function ( PSh, F, G )
        ...
        return ForAll( objects, o -> IsEqualForObjects( D, F( o ), G( o ) ) ) &&
    ...
    if (CanCompute( D, "IsLiftableAlongMonomorphism" ))
    ...
    if (CanCompute( D, "IsColiftableAlongEpimorphism" ))
    ...
    if (CanCompute( D, "MorphismBetweenDirectSumsWithGivenDirectSums" ))
    ...
    if (CanCompute( D, "MultiplyWithElementOfCommutativeSemiringForMorphisms" ))
    ...
    if (!( HasIsThinCategory( B ) && IsThinCategory( B ) && IsIntervalCategory( D ) ))
...
@InstallMethod( WellDefinednessForObjectsCheckDataOrFail,
  ...
  function ( B )
    ...
    return PairGAP( relations, is_respected );
...
@InstallMethod( WellDefinednessForObjectsCheckDataOrFail,
  ...
  function ( B )
    ...
    return PairGAP( pairs, is_respected );
...
@InstallMethod( WellDefinednessForObjectsCheckDataOrFail,
  ...
  function ( B )
    ...
    if (HasRelationsAmongGeneratingMorphisms( B ))
  ...
  function ( PSh )
    ...
    AddIsWellDefinedForMorphisms( PSh,
      function ( PSh, eta )
        ...
        return ForAll( objects, o -> IsWellDefinedForMorphisms( D, eta( o ) ) ) &&
          #          F(t(m)) --F(m)-> F(s(m))
          #             |                |
          #  eta_[t(m)] |                | eta_[s(m)]
          #             v                v
          ...
          ForAll( generating_morphisms,
                  function ( m )
                     return IsEqualForObjects( D, Target( F( m ) ), Source( eta( Source( m ) ) ) ) &&
                       IsEqualForObjects( D, Target( eta( Target( m ) ) ), Source( G( m ) ) ) &&
    ...
    if (WellDefinednessForObjectsCheckDataOrFail( B ) != fail)
        ...
        AddIsWellDefinedForObjects( PSh,
          function ( PSh, F )
            ...
            if (@not ForAll( objects, o -> IsWellDefinedForObjects( D, F( o ) ) ))
                return false;
            elseif (@not ForAll( generating_morphisms, m -> IsWellDefinedForMorphisms( D, F( m ) ) ))
                return false;
            elseif (@not ForAll( generating_morphisms, m -> IsEqualForObjects( D, F( Target( m ) ), Source( F( m ) ) ) ))
                return false;
            elseif (@not ForAll( generating_morphisms, m -> IsEqualForObjects( D, F( Source( m ) ), Target( F( m ) ) ) ))
                return false;
            end;
...
@InstallGlobalFunction( ADD_PROJECTIVE_STRUCTURE_TO_PRESHEAF_CATEGORY,
  function ( PSh )
    ...
    if (HasIsAbelianCategory( PSh ) && IsAbelianCategory( PSh ))
...
@InstallGlobalFunction( ADD_CARTESIAN_CLOSED_STRUCTURE_TO_PRESHEAF_CATEGORY,
  function ( PSh )
    ...
    if (IsSkeletalCategoryOfFiniteSets( D ) &&
       ...
       IsIdenticalObj( D, RangeCategoryOfHomomorphismStructure( B ) ))
...
@InstallMethod( AddAdditionalPrecompiledFunctionsToPreSheafCategory,
  ...
  function ( B, D, PSh )
    ...
    if (@not IsSkeletalCategoryOfFiniteSets( D ))
        return;
    end;
...
@InstallMethod( AddAdditionalPrecompiledFunctionsToPreSheafCategory,
  ...
  function ( B, D, PSh )
    ...
    if (IsCategoryOfRows( D ) &&
       IsHomalgRing( commutative_semiring ) &&
       HasIsFieldForHomalg( commutative_semiring ) && IsFieldForHomalg( commutative_semiring ) &&
       @not B.over_Z)
        
        if (IsQuotientOfPathAlgebra( UnderlyingQuiverAlgebra( B ) ) ||
           ( HasIsMonoidalCategory( D ) && IsMonoidalCategory( D ) &&
             HasCounit( B ) && HasComultiplication( B ) ))
  ...
  function ( PSh )
        ...
        List( SetOfObjects( B ),
          u -> List( SetOfObjects( B ),
            v -> List( BasisOfExternalHom( B, u, v ),
              function ( b )
                if (IsEqualToIdentityMorphism( b ))
                  return [];
                else
    ...
    AddEpimorphismFromProjectiveCoverObject( PSh,
      function( PSh, F )
        ...
        if (HasEpimorphismFromProjectiveCoverObject( F ))
...
InstallMethodWithCache( PreSheavesOfFpEnrichedCategory,
  ...
  function ( CAP_NAMED_ARGUMENTS, B, D )
    ...
    ##
    is_computable = @IsBound( D.is_computable ) && D.is_computable == true;
      ...
      function ( name, PSh )
        return """
          function( input_arguments... )
            ...
            L = @NTupleGAP( number_of_arguments, input_arguments... );
            ...
            if (IsObjectInPreSheafCategory( L[2] ))
    ...
    create_func_object =
      function ( name, PSh )
        ...
        if (@not @IsBound( info.functorial ))
        ...
        elseif (name in [ "FiberProduct", "Pushout" ])
        ...
        elseif (name in [ "Equalizer", "Coequalizer" ])
        ...
        elseif (name in [ "KernelObject", "CokernelObject", "ImageObject", "CoimageObject" ])
        ...
        @rec( sequence_of_arguments_objB =
             List( (2):(Length( info.filter_list )),
                   function( i )
                     ...
                     if (type == "integer")
                         return @Concatenation( "i_arg[", StringGAP( i ), "]" );
                     elseif (type == "object")
                         return @Concatenation( "ValuesOfPreSheaf( i_arg[", StringGAP( i ), "] )[1][objB_index]" );
                     elseif (type == "morphism")
                         return @Concatenation( "ValuesOnAllObjects( i_arg[", StringGAP( i ), "] )[objB_index]" );
                     elseif (type == "list_of_objects")
                         return @Concatenation( "List( i_arg[", StringGAP( i ), "], F -> ValuesOfPreSheaf( F )[1][objB_index] )" );
                     elseif (type == "list_of_morphisms")
                         return @Concatenation( "List( i_arg[", StringGAP( i ), "], eta -> ValuesOnAllObjects( eta )[objB_index] )" );
                     elseif (type == "pair_of_morphisms")
                         return @Concatenation( "PairGAP( ValuesOnAllObjects( i_arg[", StringGAP( i ), "][1] )[objB_index], ValuesOnAllObjects( i_arg[", StringGAP( i ), "][2] )[objB_index] )" );
                     else
    ...
    if (HasIsMonoidalCategory( D ) && IsMonoidalCategory( D ))
        ...
        if (HasIsLinearClosureOfACategory( B ) && IsLinearClosureOfACategory( B ))
    ...
    if (HasIsThinCategory( B ) && IsThinCategory( B ) &&
       IsIntervalCategory( D ))
    ...
    if (HasIsLinearCategoryOverCommutativeRingWithFinitelyGeneratedFreeExternalHoms( D ) &&
        IsLinearCategoryOverCommutativeRingWithFinitelyGeneratedFreeExternalHoms( D ) &&
        HasCommutativeSemiringOfLinearCategory( D ))
    ...
    if (HasIsMonoidalCategory( D ) && IsMonoidalCategory( D ) &&
       HasIsLinearClosureOfACategory( B ) && IsLinearClosureOfACategory( B ))
    ...
    if (@IsBound( D.supports_empty_limits ))
    ...
    if (HasCommutativeSemiringOfLinearCategory( D ))
        option_record.commutative_semiring_of_linear_category = CommutativeSemiringOfLinearCategory( D );
    end;
    
    PSh = CategoryConstructor( option_record; overhead = overhead );
    ...
    if (HasIsFiniteCategory( B ) && IsFiniteCategory( B ) &&
       CanCompute( B, "SetOfObjectsOfCategory" ) &&
       HasIsPosetCategory( PSh ) && IsPosetCategory( PSh ) &&
       IsIntervalCategory( D ))
    ...
    if (HasRangeCategoryOfHomomorphismStructure( D ) &&
       MissingOperationsForConstructivenessOfCategory( D, "IsEquippedWithHomomorphismStructure" ) == [ ])
        
        H = RangeCategoryOfHomomorphismStructure( D );
        
        if (MissingOperationsForConstructivenessOfCategory( H, "IsFiniteCompleteCategory" ) == [ ])
            
            ## SetGAP the range category of the homomorphism structure of the functor category to be
            ...
            if (IsIdenticalObj( H, D ))
                ...
                if (!(HasIsAbCategory( B ) && IsAbCategory( B ) ))
                    Append( PSh.compiler_hints.category_attribute_names,
            ...
            if (IsSkeletalCategoryOfFiniteSets( D ) && IsSkeletalCategoryOfFiniteSets( H ))
    ...
    if (HasRangeCategoryOfHomomorphismStructure( PSh ) &&
       ...
       IsIdenticalObj( D, RangeCategoryOfHomomorphismStructure( PSh ) ) &&
       ( MissingOperationsForConstructivenessOfCategory( D, "IsElementaryTopos" ) == [ ] ||
         MissingOperationsForConstructivenessOfCategory( D, "IsHeytingAlgebroid" ) == [ ] ))
    ...
    if (HasRangeCategoryOfHomomorphismStructure( B ) &&
       IsIdenticalObj( RangeCategoryOfHomomorphismStructure( B ), D ) &&
       CanCompute( D, "SubobjectClassifier" ))
    ...
    if (HasIsLinearCategoryOverCommutativeRing( B ) && IsLinearCategoryOverCommutativeRing( B ))
        
        if (HasIsAdmissibleAlgebroid( B ) || check_admissibility)
            
            if (IsAdmissibleAlgebroid( B ))
...
@InstallMethod( PreSheavesOfFpEnrichedCategory,
  ...
  function( CAP_NAMED_ARGUMENTS, B )
    ...
    return PreSheavesOfFpEnrichedCategory( B, D; FinalizeCategory = FinalizeCategory, overhead = overhead, no_precompiled_code = no_precompiled_code );
...
InstallMethodWithCache( PreSheaves,
  ...
  function( CAP_NAMED_ARGUMENTS, B, D )
    ...
    if (@not CanCompute( B, "SetOfGeneratingMorphismsOfCategory" ))
        TryNextMethod( );
    end;
...
InstallMethodWithCache( PreSheaves,
  ...
  function( CAP_NAMED_ARGUMENTS, B, k )
    ...
    PSh = PreSheaves( B, kmat; FinalizeCategory = FinalizeCategory, overhead = overhead, no_precompiled_code = no_precompiled_code );
        ...
        function( name, PSh_I_I )
            ...
            return PairGAP( """
                function( input_arguments... )
                  ...
                  return ObjectConstructor( cat, PairGAP( [ ], [ ] ) );
  ...
  function ( H )
    ...
    membership_func =
      function ( sH, N )
        ...
        ## check associativity
        if (@not IsIsomorphism( H, C2_C1xC1 ))
            return false;
        end;
...
##
@InstallMethod( YonedaEmbeddingDataOfSourceCategory,
...
@InstallMethod( /,
  ...
  function( name, PSh )
    ...
    if (IsObjectInPreSheafCategory( Yc ))
        
        SetIsProjective( Yc, true );
        
    elseif (IsMorphismInPreSheafCategory( Yc ))
        
        if (CanCompute( PSh, "IsMonomorphism" ))
            IsMonomorphism( Yc );
        end;
        
        if (CanCompute( PSh, "IsSplitMonomorphism" ))
            IsSplitMonomorphism( Yc );
        end;
        
        if (CanCompute( PSh, "IsEpimorphism" ))
            IsEpimorphism( Yc );
        end;
        
        if (CanCompute( PSh, "IsSplitEpimorphism" ))
            IsSplitEpimorphism( Yc );
        end;
        
        ## IsIsomorphism == IsSplitMonomorphism and IsSplitEpimorphism
        ## we add this here in case the logic is deactivated
        if (CanCompute( PSh, "IsIsomorphism" ))
            IsIsomorphism( Yc );
        end;
...
##
@InstallMethod( ApplyObjectInPreSheafCategoryOfFpEnrichedCategoryToObject,
...
@InstallMethod( ApplyObjectInPreSheafCategoryOfFpEnrichedCategoryToMorphism,
  ...
  function ( B, PSh, F, morB )
    ...
    elseif (IsEqualToIdentityMorphism( B, morB ))
...
@InstallMethod( ApplyObjectInPreSheafCategoryOfFpEnrichedCategoryToMorphism,
  ...
  function ( B, PSh, F, morB )
    ...
    if (WasCreatedAsOppositeCategory( B ))
...
##
@InstallMethod( ApplyObjectInPreSheafCategoryOfFpEnrichedCategoryToGeneratingMorphismOrIdentity,
        "for a presheaf category, an object in it, and a CAP morphism",
        [ IsPreSheafCategoryOfFpEnrichedCategory, IsObjectInPreSheafCategoryOfFpEnrichedCategory, IsCapCategoryMorphism ],
  ...
  function ( PSh, F, morB )
    ...
    if (IsEqualToIdentityMorphism( B, morB ))
...
##
@InstallMethod( ApplyMorphismInPreSheafCategoryOfFpEnrichedCategoryToObject,
...
@InstallMethod( CoYonedaLemmaOnObjects,
  ...
  function ( PSh, F )
    ...
    s = MorphismBetweenCoproductsWithGivenCoproducts( UC,
                 A,
                 A_list_of_objects_in_UC,
                 PairGAP( map_of_sources_C, s_list_of_morphisms_in_UC ),
                 V_list_of_objects_in_UC,
                 V );
    ...
    t = MorphismBetweenCoproductsWithGivenCoproducts( UC,
                 A,
                 A_list_of_objects_in_UC,
                 PairGAP( map_of_targets_C, t_list_of_morphisms_in_UC ),
                 V_list_of_objects_in_UC,
                 V );
...
##
@InstallMethod( EmbeddingFunctorOfFiniteStrictCoproductCompletionIntoPreSheavesData,
        [ IsPreSheafCategoryOfFpEnrichedCategory ],
...
@InstallMethod( MorphismFromRepresentableByYonedaLemma,
...
##
@InstallMethod( MorphismFromCoproductOfRepresentables,
...
@InstallMethod( CoveringListOfRepresentablesUsingSplits,
  ...
  function ( H, PSh, F )
    ...
    if (@not HasIsAbelianCategory( H ) && IsAbelianCategory( H ))
        TryNextMethod( );
    end;
...
##
@InstallMethod( SectionAndComplementByCoveringListOfRepresentables,
        [ IsPreSheafCategory, IsList, IsObjectInPreSheafCategory ],
  ...
  function ( PSh, covering_list, F )
    ...
    return PairGAP( section, complement );
...
@InstallMethod( OptimizedCoYonedaLemmaOnObjects,
   ...
   function ( PSh, F )
    ...
    return ObjectConstructor( ColimitCompletionC,
                   PairGAP( PairGAP( Target( retraction ), F_VAst[1][2] ),
                         PairGAP( PreCompose( UC, F_VAst[2][1], retraction ),
...
@InstallMethod( SievesOfPathsToTruth,
  ...
  function ( iota )
    ...
    if (!(HasIsMonomorphism( iota ) && IsMonomorphism( iota )))
        TryNextMethod( );
    end;
...
@InstallMethod( SimpleObjects,
  ...
  function ( PSh )
    ...
    if (!( HasIsAdmissibleAlgebroid( B ) && IsAdmissibleAlgebroid( B ) ))
        TryNextMethod( );
    end;
