...
@InstallMethod( BooleanAlgebraOfConstructibleObjectsAsUnionOfDifferences,
  ...
  function( P )
    ...
    if (CanCompute( D, "SetOfObjectsOfCategory" ))
...
@InstallGlobalFunction( UnionOfDifferences,
  function( L )
    ...
    ars = List( L,
                 function( A )
                   local D;
                   if (IsConstructibleObjectAsUnionOfSingleDifferences( A ))
                       return List( A );
...
##
@InstallMethod( DifferenceOfSingleDifferences,
  ...
  function( D, A, B )
    ...
    return UnionOfListOfDifferences( C,
                   [ SingleDifference( D, PairGAP( A_m, Coproduct( H, [ A_s, B_m ] ) ) ),
                     SingleDifference( D, PairGAP( DirectProduct( H, [ A_m, B_s ] ), A_s ) ) ] );
...
@InstallMethod( ==,
  ...
  function( A, B )
    ...
    if (IsConstructibleObjectAsUnionOfSingleDifferences( B ))
        TryNextMethod( );
    end;
