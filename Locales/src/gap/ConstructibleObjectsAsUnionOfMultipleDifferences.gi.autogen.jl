...
##
@InstallMethod( UnionOfListOfDifferences,
...
@InstallMethod( BooleanAlgebraOfConstructibleObjectsAsUnionOfMultipleDifferences,
  ...
  function( P )
    ...
    name = @Concatenation( "BooleanAlgebraOfConstructibleObjectsAsUnionOfMultipleDifferences( ", Name( P ), " )" );
...
@InstallGlobalFunction( UnionOfMultipleDifferences,
  function( L )
    ...
    ars = List( L,
                 function( A )
                   local D;
                   if (IsConstructibleObjectAsUnionOfMultipleDifferences( A ))
                       return List( A );
                   elseif (IsConstructibleObjectAsUnionOfSingleDifferences( A ))
                       return List( A, AsMultipleDifference );
                   elseif (IsObjectInMeetSemilatticeOfMultipleDifferences( A ))
                       return A;
                   elseif (IsObjectInMeetSemilatticeOfSingleDifferences( A ))
                       return AsMultipleDifference( A );
                   elseif (IsObjectInThinCategory( A ))
                       D = A - 0;
                       if (@not IsObjectInMeetSemilatticeOfSingleDifferences( D ))
...
@InstallMethod( ==,
  ...
  function( A, B )
    ...
    if (IsConstructibleObjectAsUnionOfMultipleDifferences( B ))
        TryNextMethod( );
    end;
