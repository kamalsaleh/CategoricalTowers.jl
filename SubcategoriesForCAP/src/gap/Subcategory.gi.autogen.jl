...
##
@InstallValueConst( CAP_INTERNAL_METHOD_NAME_LIST_FOR_SUBCATEGORY,
  [
   "IsEqualForObjects",
   "IsEqualForMorphisms",
...
@InstallMethod( AsSubcategoryCell,
  ...
  function( D, object )
    ...
    if (@not IsIdenticalObj( CapCategory( object ), AmbientCategory( D ) ))
...
@InstallMethod( AsSubcategoryCell,
  ...
  function( source, morphism, range )
    ...
    if (@not IsIdenticalObj( CapCategory( morphism ), AmbientCategory( D ) ))
...
@InstallMethod( Subcategory,
  ...
  function( CAP_NAMED_ARGUMENTS, C, name )
    ...
    list_of_operations_to_install =
      DuplicateFreeList( @Concatenation( CAP_INTERNAL_METHOD_NAME_LIST_FOR_SUBCATEGORY, additional_operations_to_install ) );
    ...
    if (is_full)
        ...
        if (category_constructor_options.category_filter == fail)
          category_constructor_options.category_filter = IsCapFullSubcategory;
        end;
        
        if (category_constructor_options.category_object_filter == fail)
          category_constructor_options.category_object_filter = IsObjectInAFullSubcategory;
        end;
        
        if (category_constructor_options.category_morphism_filter == fail)
    ...
    category_constructor_options.supports_empty_limits = supports_empty_limits;
    ...
    if (CanCompute( C, "UniqueMorphism" ))
...
@InstallGlobalFunction( SubcategoryGeneratedByListOfMorphisms,
  ...
  function( CAP_NAMED_ARGUMENTS, L )
    ...
    name = name_of_subcat_subcategory;
    ...
    if (name == fail)
      ...
      if (Length( L ) > 1)
        name = @Concatenation( "Subcategory generated by ", StringGAP( Length( L ) ), " objects in ", name );
      else
...
@InstallMethod( /,
  ...
  function( name, subcategory )
    ...
    if (( IsCapCategoryObject( c ) || IsCapCategoryMorphism( c ) ) &&
       IsIdenticalObj( CapCategory( c ), C ))
