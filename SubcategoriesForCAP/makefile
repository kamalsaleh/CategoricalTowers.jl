...
uninstall:
	julia -e 'using Pkg; Pkg.rm("SubcategoriesForCAP");'

test:
	julia -e 'using Pkg; Pkg.test("SubcategoriesForCAP");'
...
git-commit:
	...
	echo -e "\033[32mCommitting changes in \"SubcategoriesForCAP\"\033[0m"; \
	git add .; \
	git commit \
		...
		echo -e "\033[33mNo changes in \"SubcategoriesForCAP\" to commit or no version bump detected.\033[0m"; \
	fi
