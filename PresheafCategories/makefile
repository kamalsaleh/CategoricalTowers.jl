...
uninstall:
	julia -e 'using Pkg; Pkg.rm("PresheafCategories");'

test:
	julia -e 'using Pkg; Pkg.test("PresheafCategories");'
...
git-commit:
	...
	echo -e "\033[32mCommitting changes in \"PresheafCategories\"\033[0m"; \
	git add .; \
	git commit \
		...
		echo -e "\033[33mNo changes in \"PresheafCategories\" to commit or no version bump detected.\033[0m"; \
	fi
