...
@InstallMethod( PreSheavesWithBounds,
  ...
  function( CAP_NAMED_ARGUMENTS, C, D, type_of_boundedness )
    ...
    if (type_of_boundedness == "lower")
        name = "PreSheavesWithLowerBounds( ";
        is_computable = false;
    elseif (type_of_boundedness == "upper")
        name = "PreSheavesWithUpperBounds( ";
        is_computable = false;
    elseif (type_of_boundedness == "both")
    ...
    # e.g., datum == PairGAP( presheaf, PairGAP( lower_bound, upper_bound ) )
    object_datum_type =
      CapJitDataTypeOfNTupleOf( 2,
              CapJitDataTypeOfObjectOfCategory( PSh ),
              CapJitDataTypeOfNTupleOf( 2, IsCyclotomic, IsCyclotomic ) ); ## IsCyclotomic allows for -infinity && infinity
    ...
    union_of_supports =
      """(function ( supports )
        ...
        if (supports == [ ])
            ...
            return PairGAP( Minimum( List( supports, s -> s[1] ) ), Maximum( List( supports, s -> s[2] ) ) );
        end;
      ...
      function ( name, xPSh )
        ...
        return PairGAP( ReplacedStringViaRecord(
            ...
            function( input_arguments... )
              ...
              return ObjectConstructor( cat, PairGAP( operation_name( sequence_of_arguments... ), bounds ) );
            ...
            @rec( sequence_of_arguments =
              List( (1):(Length( info.filter_list )),
                function ( j )
                  ...
                  if (j == 1 && type == "category")
                      return "PSh";
                  elseif (type == "object")
                      return @Concatenation( "ObjectDatum( i_arg[", StringGAP( j ), "] )[1]" );
                  elseif (type == "morphism")
                      return @Concatenation( "MorphismDatum( i_arg[", StringGAP( j ), "] )" );
                  elseif (type == "list_of_objects")
                      return @Concatenation( "List( i_arg[", StringGAP( j ), "],  o -> ObjectDatum( cat, o )[1] )" );
                  elseif (type == "list_of_morphisms")
                      return @Concatenation( "List( i_arg[", StringGAP( j ), "],  m -> MorphismDatum( cat, m ) )" );
                  else
        ...
        @rec( sequence_of_arguments =
             List( (1):(Length( info.filter_list )),
                   function( j )
                     ...
                     if (j == 1 && type == "category")
                        return "PSh";
                     elseif (type in [ "integer", "element_of_commutative_semiring_of_linear_structure" ])
                        return @Concatenation( "i_arg[", StringGAP( j ), "]" );
                     elseif (type == "object")
                        return @Concatenation( "ObjectDatum( cat, i_arg[", StringGAP( j ), "] )[1]" );
                     elseif (type == "morphism")
                        return @Concatenation( "MorphismDatum( i_arg[", StringGAP( j ), "] )" );
                     elseif (type == "list_of_objects")
                        return @Concatenation( "List( i_arg[", StringGAP( j ), "], o -> ObjectDatum( cat, o )[1] )" );
                     ...
                     elseif (type == "pair_of_morphisms")
                        return @Concatenation( "PairGAP( ", "MorphismDatum( cat, i_arg[", StringGAP( j ), "][1] )", ", MorphismDatum( cat, i_arg[", StringGAP( j ), "][2] )", " )" );
                     elseif (type == "list_of_lists_of_morphisms")
                        return @Concatenation( "List( i_arg[", StringGAP( j ), "], x -> List( x, y -> MorphismDatum( cat, y ) ) )" );
                     else
    ...
    if (@IsBound( PSh.supports_empty_limits ))
    ...
    if (HasCommutativeSemiringOfLinearCategory( PSh ))
        category_constructor_options.commutative_semiring_of_linear_category = CommutativeSemiringOfLinearCategory( PSh );
    end;
    
    xPSh = CategoryConstructor( category_constructor_options; overhead = overhead );
    ...
    if (is_computable)
        ...
        AddIsEqualForObjects( xPSh,
          function( xPSh, F, G )
            local D, PSh, F_datum, G_datum, F_func, G_func, func, l, u;
            ...
            l = Minimum( F_datum[2][1], G_datum[2][1] );
            u = Maximum( F_datum[2][2], G_datum[2][2] );
            
            return ForAll( (l):(u), func );
        ...
        AddIsEqualForMorphisms( xPSh,
          function( xPSh, phi, psi )
            local D, PSh, F, G, F_datum, G_datum, F_func, G_func,
                  phi_datum, psi_datum, phi_func, psi_func, func, l, u;
