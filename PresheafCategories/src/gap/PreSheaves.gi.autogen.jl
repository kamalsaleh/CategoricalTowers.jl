...
##
@InstallValueConst( CAP_INTERNAL_METHOD_NAME_LIST_TO_ALWAYS_INSTALL_PRIMITIVELY_FOR_PRESHEAF_CATEGORY,
        [ 
          #"CokernelColift",
...
##
@InstallValueConst( CAP_INTERNAL_METHOD_NAME_LIST_FOR_PRESHEAF_CATEGORY,
        [ 
...
##
@InstallValueConst( CAP_INTERNAL_METHOD_NAME_LIST_FOR_MONOIDAL_PRESHEAF_CATEGORY,
        [ 
...
##
@InstallValueConst( CAP_INTERNAL_METHOD_NAME_LIST_FOR_MONOIDAL_PRESHEAF_CATEGORY_WITH_DUALS,
        [ 
...
InstallMethodWithCache( PreSheaves,
  ...
  function( CAP_NAMED_ARGUMENTS, B, D )
        ...
        if (name in [ "TerminalObject", "InitialObject", "ZeroObject" ])
            ...
            return PairGAP( ReplacedStringViaRecord(
              ...
              function ( input_arguments... )
                ...
                return ObjectConstructor( cat, PairGAP( presheaf_on_objects, presheaf_on_morphisms ) );
        ...
        @rec( sequence_of_arguments_objB =
             List( (2):(Length( info.filter_list )),
                   function( i )
                     ...
                     elseif (type == "object")
                         return @Concatenation( "PairOfFunctionsOfPreSheaf( i_arg[", StringGAP( i ), "] )[1]( objB )" );
                     elseif (type == "morphism")
                         return @Concatenation( "FunctionOfPreSheafMorphism( i_arg[", StringGAP( i ), "] )( PairOfFunctionsOfPreSheaf( Source( i_arg[", StringGAP( i ), "] ) )[1]( objB ), objB, PairOfFunctionsOfPreSheaf( Target( i_arg[", StringGAP( i ), "] ) )[1]( objB ) )" );
                     elseif (type == "list_of_objects")
                         return @Concatenation( "List( i_arg[", StringGAP( i ), "], F -> PairOfFunctionsOfPreSheaf( F )[1]( objB ) )" );
                     elseif (type == "list_of_morphisms")
                         return @Concatenation( "List( i_arg[", StringGAP( i ), "], eta -> FunctionOfPreSheafMorphism( eta )( PairOfFunctionsOfPreSheaf( Source( eta ) )[1]( objB ), objB, PairOfFunctionsOfPreSheaf( Target( eta ) )[1]( objB ) ) )" );
                     elseif (type == "pair_of_morphisms")
                         return @Concatenation( "PairGAP( FunctionOfPreSheafMorphism( i_arg[", StringGAP( i ), "][1] )( PairOfFunctionsOfPreSheaf( Source( i_arg[", StringGAP( i ), "][1] ) )[1]( objB ), objB, PairOfFunctionsOfPreSheaf( Target( i_arg[", StringGAP( i ), "][1] ) )[1]( objB ) ), FunctionOfPreSheafMorphism( i_arg[", StringGAP( i ), "][2] )( PairOfFunctionsOfPreSheaf( Source( i_arg[", StringGAP( i ), "][2] ) )[1]( objB ), objB, PairOfFunctionsOfPreSheaf( Target( i_arg[", StringGAP( i ), "][2] ) )[1]( objB ) ) )" );
                     else
    ...
    if (@not IsFunction( create_func_bool ))
        for func in list_of_operations_to_install
            if (CAP_INTERNAL_METHOD_NAME_RECORD[func].return_type == "bool")
                Add( skip, func );
            end;
        end;
    end;
    ...
    if (HasCommutativeSemiringOfLinearCategory( D ))
        ...
        if (HasIsLinearCategoryOverCommutativeRingWithFinitelyGeneratedFreeExternalHoms( D ) &&
           IsLinearCategoryOverCommutativeRingWithFinitelyGeneratedFreeExternalHoms( D ))
            ...
            if (HasIsFieldForHomalg( CommutativeSemiringOfLinearCategory( D ) ) &&
               IsFieldForHomalg( CommutativeSemiringOfLinearCategory( D ) ))
    ...
    PSh = CategoryConstructor( category_constructor_options; overhead = overhead );
  ...
  function( CAP_NAMED_ARGUMENTS, I, D )
    ...
    PSh_I_I =
      CAP_INTERNAL_CONSTRUCTOR_FOR_TERMINAL_CATEGORY( @rec(
              ...
              morphism_constructor = morphism_constructor,
              morphism_datum = morphism_datum,
              range_category_of_homomorphism_structure = H ); overhead = overhead );
...
##
@InstallMethod( YonedaEmbeddingFunctionalDataOfSourceCategory,
        [ IsPreSheafCategory ],
    ...
    Yoneda_on_objs =
      function ( obj )
        ...
        Yobj = ObjectConstructor( PSh,
                        PairGAP( o -> HomomorphismStructureOnObjects( B, o, obj ),
