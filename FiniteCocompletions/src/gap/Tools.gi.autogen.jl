...
@InstallMethod( EnrichmentSpecificFiniteStrictCoproductCompletion,
  ...
  function( CAP_NAMED_ARGUMENTS, C )
    ...
    if (@not HasRangeCategoryOfHomomorphismStructure( C ))
        Error( "the category `C` has no RangeCategoryOfHomomorphismStructure\n" );
    end;
    
    return EnrichmentSpecificFiniteStrictCoproductCompletion( C, RangeCategoryOfHomomorphismStructure( C ); FinalizeCategory = FinalizeCategory );
