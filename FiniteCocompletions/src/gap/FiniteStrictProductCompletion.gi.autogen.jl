...
@InstallMethod( FiniteStrictProductCompletion,
  ...
  function( CAP_NAMED_ARGUMENTS, C )
    ...
    if (HasIsInitialCategory( C ) && IsInitialCategory( C ))
      ...
      function( PC, pair_of_int_and_list )
        ...
        return ObjectConstructor( opUopC,
                       ObjectConstructor( UopC,
                               PairGAP( pair_of_int_and_list[1],
    ...
    modeling_tower_object_datum =
      function( PC, P )
        ...
        return PairGAP( pair[1], List( pair[2], obj -> ObjectDatum( opC, obj ) ) );
    ...
    modeling_tower_morphism_datum =
      function( PC, phi )
        ...
        return PairGAP( pair_of_lists[1],
...
@InstallMethod( EmbeddingOfUnderlyingCategoryData,
  ...
  function( PC )
    ...
    embedding_on_objects =
      objC -> ObjectConstructor( PC, PairGAP( BigInt( 1 ), [ objC ] ) );
    
    embedding_on_morphisms =
      ( source, morC, target ) -> MorphismConstructor( PC, source, PairGAP( [ BigInt( 0 ) ], [ morC ] ), target );
...
@InstallMethod( /,
  ...
  function( name, PC )
    ...
    if (IsObjectInFiniteStrictProductCompletion( Yc ))

        #TODO: is this true?
        #SetIsInjective( Yc, true );
        
    elseif (IsMorphismInFiniteStrictProductCompletion( Yc ))
        
        if (CanCompute( PC, "IsMonomorphism" ))
            IsMonomorphism( Yc );
        end;
        
        if (CanCompute( PC, "IsSplitMonomorphism" ))
            IsSplitMonomorphism( Yc );
        end;
        
        if (CanCompute( PC, "IsEpimorphism" ))
            IsEpimorphism( Yc );
        end;
        
        if (CanCompute( PC, "IsSplitEpimorphism" ))
            IsSplitEpimorphism( Yc );
        end;
        
        ## IsIsomorphism == IsSplitMonomorphism and IsSplitEpimorphism
        ## we add this here in case the logic is deactivated
        if (CanCompute( PC, "IsIsomorphism" ))
            IsIsomorphism( Yc );
        end;
...
##
@InstallMethod( ExtendFunctorToFiniteStrictProductCompletionData,
  ...
  function( PC, pair_of_funcs, category_with_strict_products )
    ...
    extended_functor_on_morphisms =
      function( source, morPC, target )
        ...
        if (@not IsEqualForObjects( category_with_strict_products, source, DirectProduct( category_with_strict_products, source_diagram ) ))
            Error( "source and DirectProduct( source_diagram ) do not coincide\n" );
        end;
        
        if (@not IsEqualForObjects( category_with_strict_products, target, DirectProduct( category_with_strict_products, target_diagram ) ))
