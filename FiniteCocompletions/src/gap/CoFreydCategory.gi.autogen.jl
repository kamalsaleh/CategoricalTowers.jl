...
@InstallMethod( /,
  ...
  function( name, coFreyd )
    ...
    if (IsCoFreydCategoryObject( Yc ))
        
        #TODO: is this true?
        #SetIsProjective( Yc, true );
        
    elseif (IsCoFreydCategoryMorphism( Yc ))
        
        if (CanCompute( coFreyd, "IsMonomorphism" ))
            IsMonomorphism( Yc );
        end;
        
        if (CanCompute( coFreyd, "IsSplitMonomorphism" ))
            IsSplitMonomorphism( Yc );
        end;
        
        if (CanCompute( coFreyd, "IsEpimorphism" ))
            IsEpimorphism( Yc );
        end;
        
        if (CanCompute( coFreyd, "IsSplitEpimorphism" ))
            IsSplitEpimorphism( Yc );
        end;
        
        ## IsIsomorphism == IsSplitMonomorphism and IsSplitEpimorphism
        ## we add this here in case the logic is deactivated
        if (CanCompute( coFreyd, "IsIsomorphism" ))
            IsIsomorphism( Yc );
        end;
...
##
@InstallMethod( ExtendFunctorToCoFreydCategoryData,
  ...
  function( KC, pair_of_funcs, additive_category_with_kernels )
    ...
    extended_functor_on_morphisms =
      function( source, morKC, target )
        ...
        if (@not IsEqualForObjects( additive_category_with_kernels, source, KernelObject( additive_category_with_kernels, source_diagram ) ))
            Error( "source and KernelObject( source_diagram ) do not coincide\n" );
        end;
        
        if (@not IsEqualForObjects( additive_category_with_kernels, target, KernelObject( additive_category_with_kernels, target_diagram ) ))
