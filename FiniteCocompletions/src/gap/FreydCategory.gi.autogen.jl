...
@InstallMethod( /,
  ...
  function( name, Freyd )
    ...
    if (IsFreydCategoryObject( Yc ))
        
        #TODO: is this true?
        #SetIsProjective( Yc, true );
        
    elseif (IsFreydCategoryMorphism( Yc ))
        
        if (CanCompute( Freyd, "IsMonomorphism" ))
            IsMonomorphism( Yc );
        end;
        
        if (CanCompute( Freyd, "IsSplitMonomorphism" ))
            IsSplitMonomorphism( Yc );
        end;
        
        if (CanCompute( Freyd, "IsEpimorphism" ))
            IsEpimorphism( Yc );
        end;
        
        if (CanCompute( Freyd, "IsSplitEpimorphism" ))
            IsSplitEpimorphism( Yc );
        end;
        
        ## IsIsomorphism == IsSplitMonomorphism and IsSplitEpimorphism
        ## we add this here in case the logic is deactivated
        if (CanCompute( Freyd, "IsIsomorphism" ))
            IsIsomorphism( Yc );
        end;
...
##
@InstallMethod( ExtendFunctorToFreydCategoryData,
  ...
  function( FC, pair_of_funcs, additive_category_with_cokernels )
    ...
    extended_functor_on_morphisms =
      function( source, morFC, target )
        ...
        if (@not IsEqualForObjects( additive_category_with_cokernels, source, CokernelObject( additive_category_with_cokernels, source_diagram ) ))
            Error( "source and CokernelObject( source_diagram ) do not coincide\n" );
        end;
        
        if (@not IsEqualForObjects( additive_category_with_cokernels, target, CokernelObject( additive_category_with_cokernels, target_diagram ) ))
