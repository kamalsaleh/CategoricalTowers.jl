...
InstallMethodWithCrispCache( CreateAmbientLinearClosureOfFpAlgebra,
  ...
  function( B, nrgens, var_name )
    ...
    nmgens = ParseListOfIndeterminates( [ @Concatenation( var_name, "1..", StringGAP( nrgens ) ) ] );
...
@InstallMethod( CategoryOfFpAlgebras,
        ...
        [ IsCapCategory && IsLinearCategoryOverCommutativeRing ],
  ...
  function( CAP_NAMED_ARGUMENTS, linear_category )
    ...
    AddObjectConstructor( FpAlg_k,
      function( FpAlg_k, septuple_lincat_uniqueobj_nrgens_gens_nrrels_rels_nmgens )
        ...
        @Assert( 0,
                IsPathCategory( UnderlyingCategory( L ) ) &&
                HasCommutativeRingOfLinearCategory( L ) &&
    ...
    AddIsEqualForObjects( FpAlg_k,
      function( FpAlg_k, fp_algebra1, fp_algebra2 )
        ...
        return IsIdenticalObj( L, datum2[1] ) &&
               IsEqualForObjects( L, datum1[2], datum2[2] ) &&
               datum1[3] == datum2[3] &&
               ForAll( (1):(datum1[3]), i -> IsEqualForMorphisms( L, datum1[4][i], datum2[4][i] ) ) &&
               ForAll( (1):(datum1[3]), i -> datum1[7][i] == datum2[7][i] ) &&
               datum1[5] == datum2[5] &&
    ...
    AddIsEqualForMorphisms( FpAlg_k,
      function( FpAlg_k, fp_algebra_morphism1, fp_algebra_morphism2 )
        ...
        return Length( list_of_images1 ) == Length( list_of_images2 ) &&
    ...
    AddIsWellDefinedForObjects( FpAlg_k,
      function( FpAlg_k, fp_algebra )
        ...
        return IsPathCategory( UnderlyingCategory( datum[1] ) ) &&
               HasCommutativeRingOfLinearCategory( datum[1] ) &&
               IsIdenticalObj( k, CommutativeRingOfLinearCategory( datum[1] ) ) &&
               IsIdenticalObj( CapCategory( datum[2] ), datum[1] ) &&
               Length( datum[4] ) == datum[3] &&
               ForAll( datum[4], gen -> IsIdenticalObj( CapCategory( gen ), datum[1] ) ) &&
               Length( datum[6] ) == datum[5] &&
               ForAll( datum[6], rel -> IsIdenticalObj( CapCategory( rel ), datum[1] ) ) &&
    ...
    AddIsWellDefinedForMorphisms( FpAlg_k,
      function( FpAlg_k, fp_algebra_morphism )
        ...
        if (@not Length( list_of_images ) == datumS[3])
            return false;
        elseif (@not ForAll( list_of_images, image -> IsIdenticalObj( CapCategory( image ), datumT[1] ) ))
            return false;
        end;
        ...
        if (@not bool && @not obstruction == fail)
            Add( obstruction, PairGAP( reds, "IsWellDefinedForMorphisms" ) );
            return true;
        end;
    ...
    AddDirectProduct( FpAlg_k,
      function( FpAlg_k, diagram )
        ...
        nmgens = ParseListOfIndeterminates( [ @Concatenation( var, "1..", StringGAP( nrgens ) ) ] );
...
@InstallMethod( Dimension,
  ...
  function( fp_algebra )
    ...
    if (HasIsEquippedWithHomomorphismStructure( quotient ) && IsEquippedWithHomomorphismStructure( quotient ) &&
       HasRangeCategoryOfHomomorphismStructure( quotient ) &&
       MissingOperationsForConstructivenessOfCategory( quotient, "IsEquippedWithHomomorphismStructure" ) == [ ] )
...
@InstallMethod( /,
        ...
        [ IsCapCategory && IsLinearCategoryOverCommutativeRing, IsCategoryOfFpAlgebras ],
        
  function( fp_linear_category_on_one_object, FpAlg_k )
    local k, L, relations, object, generators, get_labels, fp_algebra;
    
    @Assert( 0, IsLinearClosure( fp_linear_category_on_one_object ) || IsQuotientCategory( fp_linear_category_on_one_object ) );
    
    k = CommutativeRingOfLinearCategory( fp_linear_category_on_one_object );
    
    if (IsLinearClosure( fp_linear_category_on_one_object ))
        L = fp_linear_category_on_one_object;
        relations = [ ];
    else
        @Assert( 0, HasAmbientCategory( fp_linear_category_on_one_object ) && HasDefiningRelations( fp_linear_category_on_one_object ) );
    ...
    get_labels =
      function( gen )
        ...
        @Assert( 0, Length( coef ) == 1 && coef[1] in k && IsOne( coef[1] ) );
    ...
    if (HasRangeCategoryOfHomomorphismStructure( fp_linear_category_on_one_object ) &&
       IsIdenticalObj( RangeCategoryOfHomomorphismStructure( fp_linear_category_on_one_object ), UnderlyingCategoryOfMatrices( FpAlg_k ) ))
  ...
  function( FpAlg_k, fp_algebra_morphism )
    ...
    return ExtendFunctorToAlgebroidData(
                   ...
                   ExtendFunctorToFpCategoryData(
                           path_cat,
                           ## quiver_of_S → T
                           PairGAP( obj_in_quiver_S -> image_of_unique_object[obj_in_quiver_S],
...
##
@InstallMethod( EvaluateFpAlgebraMorphism,
...
@InstallMethod( Comultiplication,
  ...
  function( fp_algebra, list_of_images_of_comult )
    ...
    if (ForAny( list_of_images_of_comult, image -> IsList( image ) && ForAny( image, IsRingElement ) ))
        ...
        normalize_input =
          function( image )
            if (IsList( image ) && Length( image ) == d^2 && ForAll( image, IsRingElement ))
                return List( (1):(d^2), i -> PairGAP( image[i] * basis_pairs[i][1], basis_pairs[i][2] ) );
            else
                return image;
            end;
        end;
    ...
    @Assert( 0, ForAll( list_of_images_of_comult, list -> ForAll( list, pair -> IsList( pair ) && Length( pair ) == 2 && ForAll( pair, IsLinearClosureMorphism ) ) ) );
