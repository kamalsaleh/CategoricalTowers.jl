...
@InstallMethod( AddBialgebroidStructure,
  ...
  function( B, counit_images, comult_images )
    ...
    vertices = List( Vertices( UnderlyingQuiver( B ) ), StringGAP );
    ...
    for a in NamesOfComponents( counit_images )
        if (@not IsCapCategoryMorphism( counit_images[a] ))
    ...
    if (@not IsIdenticalObj( B0, AsCapCategory( Target( counit_functor ) ) ))
    ...
    if (@not IsIdenticalObj( B2, AsCapCategory( Target( comult_functor ) ) ))
...
@InstallMethod( AddAntipode,
  ...
  function( B, S_images_of_generating_morphisms )
    ...
    if (@not IsIdenticalObj( B, AsCapCategory( Target( S_functor ) ) ))
...
@InstallMethod( IsCoassociative,
  ...
  function( B )
    ...
    for m in morphisms
        ...
        if (@not ImageElm( iso1, underlying_quiver_algebra_element_of_comult_times_id_after_comult_of_m) == ImageElm( iso2, underlying_quiver_algebra_element_of_id_times_comult_after_comult_of_m))
...
@InstallMethod( IsCoassociative,
  ...
  function( B )
    ...
    if (@not HasComultiplication( B_as_category ))
...
@InstallMethod( IsCounitary,
        ...
        [ IsAlgebraAsCategory && IsCommutative && IsBialgebroid ],
  ...
  function( B )
    ...
    for o in SetOfObjects( B )

        if (!( ( IsEqualForObjects( ApplyFunctor(comp1, o), o ) ) && ( IsEqualForObjects( ApplyFunctor(comp2, o), o ) ) ))
    ...
    for m in SetOfGeneratingMorphisms( B )

        if (!( ( IsEqualForMorphisms( ApplyFunctor(comp1, m), m ) ) && ( IsEqualForMorphisms( ApplyFunctor(comp2, m), m ) ) ))
...
@InstallMethod( IsCounitary,
  ...
  function( B )
    ...
    if (@not HasCounit( B_as_category ))
    ...
    return IsCongruentForMorphisms(comp1, id) && IsCongruentForMorphisms(comp2,id);
end );
...
@InstallMethod( IsCocommutative,
  ...
  function( B )
    ...
    for o in SetOfObjects( B )
        ...
        if (@not IsEqualForObjects( ApplyFunctor(twist_after_comult, o), ApplyFunctor(comult, o) ))
    ...
    for m in SetOfGeneratingMorphisms( B )
        ...
        if (@not IsEqualForMorphisms( ApplyFunctor(twist_after_comult, m), ApplyFunctor(comult, m ) ))
...
@InstallMethod( IsHopfAlgebroid,
  ...
  function( B )
    ...
    for o in SetOfObjects( B )
        ...
        if (!( IsEqualForObjects( ApplyFunctor(composition1, o), ApplyFunctor(unit_after_counit, o) ) && IsEqualForObjects( ApplyFunctor(composition2, o), ApplyFunctor(unit_after_counit, o) ) ))
    ...
    for m in SetOfGeneratingMorphisms( B )
        ...
        if (!( IsEqualForMorphisms( ApplyFunctor(composition1, m), ApplyFunctor(unit_after_counit, m ) ) && IsEqualForMorphisms( ApplyFunctor(composition2, m), ApplyFunctor(unit_after_counit, m) ) ))
