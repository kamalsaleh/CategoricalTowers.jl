  ...
  function( category )
    ...
    AddIsCongruentForMorphisms( category,
      function( cat, morphism_1, morphism_2 )
        ...
        for o in objects_in_source1
            if (@not IsEqualForObjects( ApplyFunctor( morphism_1_underlying_functor, o ),
                                      ApplyFunctor( morphism_2_underlying_functor, o ) ))
                return false;
            end;
        end;
  ...
  function( F, G )
    ...
    for a in SF_objects
      for b in SG_objects
        ...
        record_with_images_of_objects[StringGAP( UnderlyingVertex( axb] ) ) = ElementaryTensor( Fa, Gb, TFxTG );
    ...
    for a in SF_objects
      for g in SG_mor
        ...
        if (@not IsPathAlgebraElement( axg ))
        
          axg = Representative( axg );

        end;
        
        axg = StringGAP( LeadingPath( axg ) );
    ...
    for f in SF_mor
      for b in SG_objects
        ...
        if (@not IsPathAlgebraElement( fxb ))

          fxb = Representative( fxb );

        end;

        fxb = StringGAP( LeadingPath( fxb ) );
...
@InstallMethod( LeftUnitorAsFunctor,
  ...
  function( A )
    ...
    for o in objects_in_A
        unitor_functor_images_of_objects[StringGAP(UnderlyingVertex(ElementaryTensor( unique_object_in_k, o, kxA ] ) ) ) = o;
    end;
    
    unitor_functor_images_of_generating_morphisms = @rec();
    for m in SetOfGeneratingMorphisms( A )
        s = StringGAP( Paths( UnderlyingQuiverAlgebraElement( ElementaryTensor( unique_object_in_k, m, kxA) ) )[1] );
...
@InstallMethod( RightUnitorAsFunctor,
  ...
  function( A )
    ...
    unitor_functor_images_of_objects = @rec( );
    for o in objects_in_A
        unitor_functor_images_of_objects[StringGAP(UnderlyingVertex(ElementaryTensor( o, unique_object_in_k, Axk ] ) ) ) = o;
    end;
    
    unitor_functor_images_of_generating_morphisms = @rec();
    for m in SetOfGeneratingMorphisms( A )
        s = StringGAP( Paths( UnderlyingQuiverAlgebraElement( ElementaryTensor( m, unique_object_in_k, Axk ) ) )[1] );
        unitor_functor_images_of_generating_morphisms[s] = m;
    end;
...
@InstallMethod( LeftUnitorInverseAsFunctor,
  ...
  function( A )
    ...
    for o in objects_in_A
        unitor_inverse_functor_images_of_objects[StringGAP(UnderlyingVertex(o])) = ElementaryTensor( unique_object_in_k, o, kxA );
    end;
    
    unitor_inverse_functor_images_of_generating_morphisms = @rec();
    for m in SetOfGeneratingMorphisms( A )
        s = StringGAP( Paths(UnderlyingQuiverAlgebraElement(m))[1] );
...
@InstallMethod( RightUnitorInverseAsFunctor,
  ...
  function( A )
    ...
    unitor_inverse_functor_images_of_objects = @rec( );
    for o in objects_in_A
        unitor_inverse_functor_images_of_objects[StringGAP(UnderlyingVertex(o])) = ElementaryTensor( o, unique_object_in_k, Axk );
    end;
...
@InstallMethod( AssociatorLeftToRightWithGivenTensorProductsAsFunctor,
  ...
  function( AxB_C, A, B, C, A_BxC )
    ...
    for m in SetOfGeneratingMorphisms( AxB_C )
        m_flat = ImageElm( iso_to_flat, UnderlyingQuiverAlgebraElement(m) );
        m_right = ImageElm( iso_from_flat, m_flat );
        m_string = StringGAP(Paths(UnderlyingQuiverAlgebraElement(m))[1]);
        m_right_string = StringGAP(Paths(m_right)[1]);
        functor_images_of_generating_morphisms[m_string] = A_BxC[m_right_string];
    end;
    for o in SetOfObjects( AxB_C )
        o_string = StringGAP( UnderlyingVertex( o ) );
        ...
        o_right_string = StringGAP(Paths(o_right)[1]);
...
@InstallMethod( Twist,
  ...
  function( A, B )
    ...
    for a in A_objects
      for b in B_objects
        record_with_images_of_objects[StringGAP( UnderlyingVertex( ElementaryTensor( a, b, AxB] ) ) ) = ElementaryTensor( b, a, BxA );
      end;
    end;
