...
##
@InstallMethod( AssignSetOfObjects,
        [ IsFpCategory && HasUnderlyingQuiver ],
  function( A )
...
InstallMethodWithCache( SetOfGeneratingMorphisms,
        ...
        [ IsFpCategory && HasUnderlyingQuiver, IsObjectInFpCategory, IsObjectInFpCategory ],
...
InstallMethodWithCache( SetOfGeneratingMorphisms,
        ...
        [ IsFpCategory && HasUnderlyingQuiver, IsInt, IsInt ],
...
@InstallMethod( RelationsAmongGeneratingMorphisms,
  ...
  function( C )
    ...
    return List( relations, pair -> PairGAP( func( pair[1] ), func( pair[2] ) ) );
...
@InstallMethod( DecompositionIndicesOfMorphism,
  ...
  function( C, mor )
    ...
    if (IsEqualToIdentityMorphism( C, mor ))
    ...
    #% CAP_JIT_DROP_NEXT_STATEMENT
    if (ForAny( cmp[2][1], IsCapCategoryObject ))
  ...
  function( category )
    ...
    AddIsWellDefinedForObjects( category,
      function( category, o )
        ...
        return IsQuiverVertex( o ) && IsIdenticalObj( QuiverOfPath( o ), UnderlyingQuiver( category ) );
    ...
    AddIsWellDefinedForMorphisms( category,
      function( category, alpha )
        ...
        if (@not IsIdenticalObj( AlgebraOfElement( m ), UnderlyingQuiverAlgebra( category ) ))
        ...
        if (!( UnderlyingVertex( Source( alpha ) ) == v ))
        ...
        if (!( UnderlyingVertex( Target( alpha ) ) == w ))
...
@InstallMethod( BasisPathOfPathAlgebraBasisElement,
  ...
  function( basis_element )
    ...
    @Assert( 0, Length( paths ) == 1 && Coefficients( basis_element ) == [ 1 ] );
...
@InstallGlobalFunction( ADD_FUNCTIONS_FOR_HOM_STRUCTURE_OF_FP_CATEGORY,
  function( fpcategory )
    ...
    if (IsSkeletalCategoryOfFiniteSets( RangeCategoryOfHomomorphismStructure( fpcategory ) ))
...
InstallMethodWithCache( Category,
  ...
  function( A, range_category_of_HomStructure )
    ...
    C = @Concatenation( C, parity, "Quiver( \"", StringGAP( quiver ), "\" ) )" );
    
    relations = CAP_INTERNAL_RETURN_OPTION_OR_DEFAULT( "relations", [ ] );
    
    if (@not relations == [ ])
        
        if (Length( relations ) < 6)
            
            rels = @Concatenation(
                            " [",
                            JoinStringsWithSeparator( List( relations, a -> @Concatenation( " ", StringGAP( a[1] ), " == ", StringGAP( a[2] ) ) ) ),
                            " ]" );
...
@InstallMethod( Category,
  ...
  function( Qq, L )
    ...
    if (!( IsList( L ) &&
             ForAll( L, IsList ) &&
             ForAll( L, a -> Length( a ) == 2 ) &&
             ForAll( L, a -> ForAll( a, IsPath ) ) ))
...
@InstallMethod( QuotientCategory,
        ...
        [ IsFpCategory && HasUnderlyingQuiver, IsList ],
        
  function( C, L )
    local relations, f;
    
    if (HasRelationsOfFpCategory( C ))
...
@InstallMethod( Multiplication,
        ...
        [ IsMonoidAsCategory && IsCommutative ],
...
@InstallMethod( MorphismInFpCategory,
  ...
  function( B, S, path, T )
    ...
    if (@not IsIdenticalObj( CapCategory(S), CapCategory(T) ))
...
@InstallMethod( MorphismInFpCategory,
  ...
  function( C, path )
    ...
    elseif (@not Coefficients( path ) == [ 1 ])
...
@InstallMethod( ^,
        ...
        [ IsFpCategory && HasUnderlyingQuiverAlgebra, IsInt ],
  ...
  function( C, n )
    ...
    if (n == 0)
        ...
        if (@not @IsBound( C.powers.0 ))
            
            if (IsRightQuiverAlgebra( Qq ))
                parity = "Right";
            else
                parity = "Reft";
            end;
    ...
    if (@not @IsBound( C.powers[n] ))
        if (@not @IsBound( C.powers[n-1] ))
            C.powers[n-1] = C^(n-1);
        end;
...
@InstallMethod( *,
        "for two categorys",
        [ IsFpCategory && HasUnderlyingQuiverAlgebra, IsFpCategory && HasUnderlyingQuiverAlgebra ],
...
@InstallMethod( OppositeFpCategory,
        "for a f.p. category",
        [ IsFpCategory && HasRelationsOfFpCategory ],
...
@InstallMethod( ViewObj,
  ...
  function( m )
    ...
    if (IsRightQuiverAlgebra( UnderlyingQuiverAlgebra( CapCategory( m ) ) ))
