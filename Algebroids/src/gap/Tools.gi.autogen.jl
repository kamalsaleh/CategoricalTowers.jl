...
@InstallMethod( Dimension,
        "for an algebroid",
        [ IsCapCategory && IsObjectFiniteCategory && IsLinearCategoryOverCommutativeRingWithFinitelyGeneratedFreeExternalHoms ],
  ...
  function( q )
    ...
    return Immutable(
                   Triple( Length( vertices ),
                           Length( arrows ),
                           List( arrows, m -> PairGAP( -1 + SafePosition( vertices, Source( m ) ), -1 + SafePosition( vertices, Target( m ) ) ) ) ) );
...
@InstallMethod( \.,
  ...
  function( UC, string_as_int )
    ...
    if (IsAdditiveClosureObject( Yc ))

        #TODO: is this true?
        #SetIsProjective( Yc, true );
        
    elseif (IsAdditiveClosureMorphism( Yc ))
