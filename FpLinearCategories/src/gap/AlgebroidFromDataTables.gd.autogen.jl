...
@DeclareFilter( "IsQuotientCategoryOfFpAlgebroidFromDataTables",
...
@DeclareFilter( "IsObjectInQuotientCategoryOfFpAlgebroidFromDataTables",
...
@DeclareFilter( "IsMorphismInQuotientCategoryOfFpAlgebroidFromDataTables",
        FilterIntersection( IsQuotientCategoryMorphism, IsMorphismInFpLinearCategory ) );

#! @Description
#!  The arguments are an algebroid $A$, defined by finite data tables, and a
#!  list <A>I</A> of morphisms in $A$.  The output is the quotient category
#!  $A/\langle I\rangle$, where $\langle I\rangle$ is the two-sided ideal
#!  generated by <A>I</A>.
#!
#!  The implementation regards the Hom-bifunctor of $A$ as a presheaf on
#!  $A^[\mathrm[op]]\otimes A$.  The relations define, by the Yoneda lemma, a
#!  morphism from a direct sum of representable presheaves into this
#!  Hom-bifunctor.  Its image at $v^[\mathrm[op]]\otimes u$ is
#!  $\langle I\rangle(u,v)$, so the corresponding cokernel presents
#!  $\operatorname[Hom]_[A/\langle I\rangle](u,v)$.
#!
#!  All these pointwise maps are finite matrices obtained from the data
#!  tables of $A$.  Hence ideal membership and quotient Hom computations use
#!  linear algebra (cokernels over a field, and lifts or Freyd-category
#!  presentations over more general computable rings).  In particular, this
#!  construction computes the quotient directly by linear algebra and avoids
#!  computing a noncommutative Groebner basis for the two-sided ideal
#!  generated by <A>I</A>.
#! @Arguments A, I
#! @Returns a &CAP; category
